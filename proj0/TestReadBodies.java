/**
 *  Tests NBody.readBodies. Reads in ./data/Bodys.txt and checks output of
 *  readBodies().
 */
public class TestReadBodies {
    ...
    private static String checkReadBodys() {
        ...
        if (missingBodys.length() > 0) {
            String answer = "FAIL: readBodies(); Missing these Bodys: ";
            answer += missingBodys.substring(0, missingBodys.length() - 2);
            return answer;
        }
        if (!randomChecksOkay) {
            return "FAIL: readBodies(); Not all Bodys have correct info!";
        }
